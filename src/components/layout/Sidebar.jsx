...
const Sidebar = () => {
  const [activeMainTab, setActiveMainTab] = useState('security'); // 'security' (maletín) por defecto según imagen
  ...
  return (
    <div className="flex flex-col h-screen bg-[#191A1A] text-gray-300 w-72 flex-shrink-0">
      
      {/* Header (Full Width) */}
      <div className="h-16 flex items-center justify-between px-4 border-b border-gray-800 flex-shrink-0">
        <div className="flex items-center gap-2">
           <img src="src/assets/logo-fastcloud-dark.png" alt="FastCloud" className="h-10 w-auto object-contain" />
           <span className="text-gray-500 text-xs mt-1 ml-2">V 1.0.0</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="w-16 flex flex-col items-center py-4 bg-[#191A1A] border-r border-gray-800 z-20 overflow-y-auto">
          ...
          <div className="flex flex-col gap-4 w-full">
            {primaryItems.map((item) => (
              <button 
                ...
                className={`relative w-full flex items-center justify-center p-3 rounded-xl transition-all group ${activeMainTab === item.id ? 'bg-[#2D3748] text-[#EC6317]' : 'text-gray-400 hover:text-white'}`}
              >
                 {activeMainTab === item.id && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-[#EC6317] rounded-r-full"></div>
                 )}
      ...
      {/* Secondary Sidebar (Menu Content) */}
      <div className="flex-1 flex flex-col bg-[#191A1A] overflow-hidden">
          ...
          <div className="mb-1">
            ...
            {openSubmenus.procesos && (
              <div className="pl-3 mt-0.5 space-y-0.5">
                ...
                <Link to="/dashboard/menu-rol" className="flex items-center gap-2 p-1.5 rounded-lg bg-[#EC6317] text-white transition-colors text-sm">
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 border-t border-gray-800 bg-[#191A1A] z-30">
            ...
            <div className="w-full h-1 bg-gray-700 rounded-full mb-1 overflow-hidden">
                <div className="h-full bg-[#EC6317] rounded-full" style={{ width: '65%' }}></div>
            </div>
